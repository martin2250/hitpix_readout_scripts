...
def main(
    ...
    } | hitpix.defaults.voltages_default

    ############################################################################

    def config_from_scan_dict(scan_dict: dict) -> SCurveConfig:
        return SCurveConfig(
    ...
    if scan_parameters:
        with h5py.File(path_output, 'a') as file:
            ...
            for idx in np.ndindex(*scan_shape):
                ...
                if group_name in file:
                    ...
                    continue
                # apply scan and set
                scan_dict = copy.deepcopy(scan_dict_template)
                util.gridscan.apply_scan(scan_dict, scan_parameters, idx)
                util.gridscan.apply_set(scan_dict, args_set)
                # extract all values
                config = config_from_scan_dict(scan_dict)
                # perform measurement
                prog_meas.reset()
        ...
        util.gridscan.apply_set(scan_dict_template, args_set)
        config = config_from_scan_dict(scan_dict_template)
